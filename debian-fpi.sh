...
setupDesktopEnviroment() {
    ...
    if [ ${desktopEnviroment} = "gnome" ]; then
        apt-get install -yy gdm3* > /dev/null 2>&1
    elif [ ${desktopEnviroment} = "kde" ]; then
        apt-get install -yy sddm* > /dev/null 2>&1 && apt-get purge -yy discover plasma-discover kinfocenter xterm --autoremove > /dev/null 2>&1
    elif [ ${desktopEnviroment} = "xfce" ]; then
        apt-get install -yy xfce4 > /dev/null 2>&1
        else
...
setupBasicSystemTools() {
    ...
    apt-get install -yy ${bstPkgs} > /dev/null 2>&1
...
setupBasicTools() {
    ...
    if [ ${wbInstallationWarningAccepted} = "yes" ]; then
        ...
        apt-get update > /dev/null 2>&1 && apt-get install -yy ${btPkgs} > /dev/null 2>&1
        else
...
setupWebBrowser() {
    ...
    if [ ${wbInstallationWarningAccepted} = "yes" ]; then
        ...
        if [ ${wbPkgs} = *"google-chrome-stable"* ]; then
            ...
            apt-get update > /dev/null 2>&1 && apt-get install -yy google-chrome-stable > /dev/null 2>&1
            ...
            apt-get install -yy apt-transport-https curl > /dev/null 2>&1
        ...
        apt-get update > /dev/null 2>&1 && apt-get install -yy ${wbPkgs} > /dev/null 2>&1
        else
            echo "${warningPromptDisagrement}" && echo ""
    fi
}
...
setupOfficeSuite() {
    ...
    if [ ${osInstallationWarningAccepted} = "yes" ]; then
        ...
        apt-get install -yy libreoffice > /dev/null 2>&1
        else
            echo "${warningPromptDisagrement}" && echo ""
    fi
}
...
setupGamingSoftware() {
    ...
    if [ ${gsInstallationWarningAccepted} = "yes" ]; then
        ...
        if [ ${gsPkgs} = *"lutris"* ]; then
            ...
            apt-get install -yy lutris > /dev/null 2>&1
        ...
        apt-get update > /dev/null 2>&1 && apt-get install -yy --install-recommends ${wbPkgs} > /dev/null 2>&1
        else
            echo "${warningPromptDisagrement}" && echo ""
    fi
}
...
setupMultimediaSoftware() {
    ...
    if [ ${msInstallationWarningAccepted} = "yes" ]; then
        ...
        apt update > /dev/null 2>&1 && apt-get install -yy ${msPkgs} > /dev/null 2>&1
        else
            echo "${warningPromptDisagrement}" && echo ""
    fi
}
...
setupDeveloperSoftware() {
    ...
    if [ ${dsInstallationWarningAccepted} = "yes" ]; then
        ...
        apt-get update > /dev/null 2>&1 && apt-get install -yy ${dsPkgs} > /dev/null 2>&1
        else
            echo "${warningPromptDisagrement}" && echo ""
    fi
}
...
setupLatestNvidiaDrivers() {
    ...
    if [ ${lndInstallationWarningAccepted} = "yes" ]; then
        ...
        apt-get install -yy ${lndPkgs} > /dev/null 2>&1
        else
            echo "${warningPromptDisagrement}" && echo ""
    fi
}
...
setupOpenSourceDrivers() {
    ...
    if [ ${osdInstallationWarningAccepted} = "yes" ]; then
        ...
        apt-get install -yy ${osdPkgs} > /dev/null 2>&1
        else
            echo "${warningPromptDisagrement}" && echo ""
    fi
}
